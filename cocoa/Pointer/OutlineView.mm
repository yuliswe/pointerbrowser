...
- (id)initWithCoder:(NSCoder *)coder
{
    ...
    self.hovered = NO;
    self.tracking_area = [[NSTrackingArea alloc] initWithRect:self.bounds options:NSTrackingMouseEnteredAndExited|NSTrackingActiveInKeyWindow|NSTrackingMouseMoved|NSTrackingInVisibleRect owner:self userInfo:nil];
...
- (instancetype)initWithWebpage:(Webpage_)w outlineView:(OutlineView*)outlineView isTemporary:(BOOL)isTemporary
{
    ...
    self.webpage = w;
    if (! isTemporary) {
        w->set_associated_frontend_tab_object_direct((__bridge void*)self);
    }
